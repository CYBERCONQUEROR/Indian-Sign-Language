...
with open("hackathon_encoder.pkl", "rb") as f:
    le = pickle.load(f)
...
# Set female voice (usually index 1, but it depends on the system)
engine.setProperty('voice', voices[1].id)  # Assuming voices[1] is the female voice

# Set speech rate if needed (optional)
engine.setProperty('rate', 150)  # You can adjust the speech rate as needed

# Set volume if needed (optional)
engine.setProperty('volume', 1)  # Set volume level to maximum (1.0)
...
# Normalize for LSTM input
def normalize_landmarks(landmarks):
    landmarks = np.array(landmarks).reshape(-1, 2)  # Shape: (42, 2)
    base = landmarks[0]
    ...
    return normalized.flatten()  # Shape: (84,) for LSTM input
...
def extract_landmarks_from_frame(frame):
    ...
    if results.multi_hand_landmarks:
        if len(results.multi_hand_landmarks) == 2:
            ...
            lms1 = [[lm.x, lm.y] for lm in results.multi_hand_landmarks[0].landmark]
            lms2 = [[0, 0]] * 21  # Pad missing hand with zeros
        combined = lms1 + lms2  # Total 42 points
        landmark_list = normalize_landmarks(combined)  # Output shape: (84,)
...
class SignRecognizerApp:
    ...
    def create_ui(self):
        ...
        tk.Button(button_frame, text="👀 See all the Signs", font=("Helvetica", 14), command=self.show_gestures, bg="#ff6347", fg="white").pack(side=tk.LEFT, padx=10)
        ...
        def listen_and_display():
            ...
            with sr.Microphone() as source:
                try:
                    ...
                    label.config(text=f"🗣️ You said: {text} (Translated: {translated_text})")

                    # Now display images based on translated text
                    display_words_with_animation(translated_text.lower())
    ...
    def show_gestures(self):
        ...
        gestures_window = tk.Toplevel(self.root)
        gestures_window.title("Gesture Images (A-Z & 0-9)")
        gestures_window.geometry("1200x600")
        gestures_window.configure(bg="#121212")

        # List of characters (A-Z, 0-9)
        gesture_chars = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
        image_folder = r"C:\Users\Krishnav\OneDrive\Desktop\AKTU\Today"  # Update with your image folder path

        # Create a canvas with a scrollbar to display the images
        canvas_frame = tk.Frame(gestures_window, bg="#121212")
        canvas_frame.pack(fill=tk.BOTH, expand=True)
        ...
        # Display the images for A-Z and 0-9
        for i, char in enumerate(gesture_chars):
            ...
            if os.path.exists(img_path):
                ...
                # Create a label for the image and character
                gesture_frame = tk.Frame(scroll_frame, bg="#121212", padx=10, pady=10, bd=2, relief="solid")
                gesture_frame.grid(row=i//6, column=i%6, padx=12, pady=12)

                img_label = tk.Label(gesture_frame, image=photo, bg="#121212")
                img_label.image = photo
                img_label.pack(side=tk.LEFT, padx=2)

                label = tk.Label(gesture_frame, text=char, font=("Helvetica", 12), fg="white", bg="#121212")
                label.pack(side=tk.LEFT, padx=2)

        # Update the scroll region after adding the images
        gestures_window.after(100, lambda: canvas.configure(scrollregion=canvas.bbox("all")))
